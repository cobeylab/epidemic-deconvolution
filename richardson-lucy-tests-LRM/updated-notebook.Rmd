...
intervention_length_name <- "_testing_clean_code_std_highly_misspecified_b"
...
### Observations - uncertainty in delay and undersampling. A value of 0 means no uncertainty. ###
      delay_distr_mean_std = .5, #the percent uncertainty in the mean of the delay distribution - specify a single value
      delay_distr_std_std = .5, #the percent uncertainty in the std of the delay distribution - specify a single value
      time_to_obs_std = 0, #the percent uncertainty in the time to observation of the delay distribution - specify a single value
      frac_sampled_std = 0, #the percent uncertainty in the fraction sampled - specify a single value
### Misspecification in observation distributions. A value of 1 means no misspecification. ###
      delay_distr_mean_mult = 1, #multiplier for how much to misspecify delay distribution mean - specify a single value
      delay_distr_std_mult = 2.0, #multiplier for how much to misspecy delay distribution std - specify a single value
      frac_sampled_mean_mult = 1.2, #multiplier for how much to misspecify fraction sampled - specify a single value
      time_to_obs_mult = 1, #multiplier for how much to misspecify time to observation - specify a single value
...
# For now, the assumed_ values are just equal to the true_ values; i.e., have not implemented misspecification in this notebook
assumed_mean <- parlist$delay_distr_mean*parlist$delay_distr_mean_mult
assumed_mean_std <- parlist$delay_distr_mean_std

assumed_std <- parlist$delay_distr_std*parlist$delay_distr_std_mult
assumed_std_std <- parlist$delay_distr_std_std

assumed_time_to_obs <- parlist$time_to_obs*parlist$time_to_obs_mult
assumed_time_to_obs_std <- parlist$time_to_obs_std

assumed_frac_sampled <- parlist$frac_sampled*parlist$frac_sampled_mean_mult
assumed_frac_sampled_std <- parlist$frac_sampled_std

# Using equations for "generation and parameters" from lognormal distribution wikipedia page; now built into obs_delays_with_uncertainty
...
```{r}
init_true_delay_distr_vec <- tabulate(obs_delays_with_uncertainty(1000000, true_mean, 0, true_std, 0, true_time_to_obs,0))
true_unnorm_delay_distr_vec <- ifelse(init_true_delay_distr_vec > 0, init_true_delay_distr_vec, 10^(-8)) #never 0; useful later
true_delay_distr_vec <- true_unnorm_delay_distr_vec/sum(true_unnorm_delay_distr_vec) * true_frac_sampled
...
init_delay_distr_vec <- tabulate(obs_delays_with_uncertainty(1000000, assumed_mean, assumed_mean_std, assumed_std, assumed_std_std, assumed_time_to_obs, assumed_time_to_obs_std))
...
make_same_length <- function(v1, v2){
  if(length(v1)<length(v2)){
    v1fix <- c(v1, rep(0, length(v2)-length(v1)))
    v2fix <- v2
  } else{
    v2fix <- c(v2, rep(0, length(v1)-length(v2)))
    v1fix <- v1
  }
  list("v1" = v1fix, "v2" = v2fix)
}

same_len_distrs <- make_same_length(true_delay_distr_vec, delay_distr_vec)

delay_distr_plot_df <- data.frame("num_days" = 1:length(same_len_distrs$v1), "true_delay_distribution" = same_len_distrs$v1, "assumed_delay_distribution" = same_len_distrs$v2) #note that num_days starts with 1, not 0, because "tabulate" bins data beginning with 1. this means we have an implicit assumption that infections are never observed on the day when they occur, which is realistic.

delay_distr_plot_df %>% 
  ggplot() +
  geom_point(aes(x = num_days, y = true_delay_distribution, col = "True delay distribution", lty = "True delay distribution", legend = "True delay distribution"), size = 1)  +
  geom_line(aes(x = num_days, y = true_delay_distribution, col = "True delay distribution", lty = "True delay distribution", legend = "True delay distribution"), size = .7)  +
  geom_point(aes(x = num_days, y = delay_distr_vec, col = "Assumed delay distribution", lty = "Assumed delay distribution", legend = "Assumed delay distribution"), size = 1)  +
  geom_line(aes(x = num_days, y = delay_distr_vec, col = "Assumed delay distribution", lty = "Assumed delay distribution", legend = "Assumed delay distribution"), size = .7)  +
  scale_color_manual(values = c("black", "blue")) + 
  scale_linetype_manual(values = c("solid", "dotted")) +
  labs(linetype = "", colour = "", fill = "") +
  ylab("number of days")+
  xlab("probability density") + 
  ggtitle('True and assumed delay distributions') -> delay_distribution_plot
delay_distribution_plot

saveRDS(delay_distribution_plot, sprintf("%s/delay_distribution_plot.Rds", intervention_file_name))
ggsave(sprintf("%s/delay_distribution_plot.png", intervention_file_name))
...
true_delay_fn <- function(nn) {
  obs_delays_with_uncertainty(nn, true_mean, 0, true_std, 0, true_time_to_obs,0)
}
...
```{r}
one_shifted_curve <- function(observed_cases, vec_of_delays, frac_observed) {
  shift_val <- ceiling(median(vec_of_delays))
  observed_cases[shift_val:length(observed_cases)]*(1/frac_observed)
}
...
get_samples_shifting_method <- function(observed_cases, n_delay_samples) {
  ...
  for (curve_count in 1:n_delay_samples) {
    frac_observed_sample <- rnorm(1, assumed_frac_sampled, assumed_frac_sampled_std)
    this_vec_of_delays <- obs_delays_with_uncertainty(length(observed_cases), assumed_mean, assumed_mean_std, assumed_std, assumed_std_std, assumed_time_to_obs, assumed_time_to_obs_std)
    print(median(this_vec_of_delays))
    print(var(this_vec_of_delays))
    next_curve <- one_shifted_curve(observed_cases, this_vec_of_delays, frac_observed_sample)
...
inf_curve_samples <- get_samples_shifting_method(sim_df$new_observed, 50)
...
shift_df %>%
  ggplot() +
  geom_line(aes(x = time, y = shift.median, col = "Median Infections, Shifted", lty = "Median Infections, Shifted", legend = "Median Infections, Shifted"), size = .6)  +
  geom_line(aes(x = time, y = true, col = "True Infections", lty = "True Infections", legend = "True Infections"), size = .6) + 
...
* 12/29/20: Changed get_samples_RL_method to allow input of observation times instead of always using parlist, with parlist values still the defaults
```{r}
get_samples_RL_method <- function(obs_curve, delay_vec, obs_1_offset_in = parlist$obs_1_offset, obs_2_offset_in = parlist$obs_2_offset){
  ...
  obs_1 <- length(p)+obs_1_offset_in +1
  obs_2 <- obs_1 + obs_2_offset_in
...
```{r}
all_inferred_df <- merge(shift_df, bayes_deconv_df, by = c("time", "true"))
all_inferred_df <- all_inferred_df[order(all_inferred_df$time),]
all_inferred_df

all_inferred_df %>%
  ggplot() +
  geom_line(aes(x = time, y = true, col = "True Infections", lty = "True Infections", legend = "True Infections"), size = .7) + 
  geom_line(aes(x = time, y = shift.median, col = "Median Infections, Shifted", lty = "Median Infections, Shifted", legend = "Median Infections, Shifted"), size = .6)  +
  geom_ribbon(aes(x = time, ymin = shift.025, ymax = shift.975, fill = "95% CI, shifted", legend = "95% CI, shifted"), alpha = 0.4) +
  geom_line(aes(x = time, y = inferred.median, col = "Median Infections, Inferred", lty = "Median Infections, Inferred", legend = "Median Infections, Inferred"), size = .6)  +
  geom_ribbon(aes(x = time, ymin = inferred.025, ymax = inferred.975, fill = "95% CI, inferred", legend = "95% CI, inferred"), alpha = 0.4) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, col = "Intervention Time", lty = "Intervention Time", legend = "Intervention Time"), size = .5) +
  scale_color_manual(values = c("black", "red", "blue", "black")) + 
  scale_linetype_manual(values = c("dotted", "solid", "solid", "dotdash")) +
  labs(linetype = "", colour = "", fill = "") +
  ylab("count")+
  xlab("time (days)") + 
  ggtitle('Shifting vs. RL inference of case counts') -> shift_RL_true_with_CI

shift_RL_true_with_CI

saveRDS(RL_inferred_plot_with_CI, sprintf("%s/shift_RL_true_with_CI.Rds", intervention_file_name))
ggsave(sprintf("%s/shift_RL_true_with_CI.png", intervention_file_name))
```

## Added 12/22/20

```{r}
diff_frac_shifted <- (all_inferred_df$shift.median-all_inferred_df$true)/all_inferred_df$true
diff_frac_RL <- (all_inferred_df$inferred.median-all_inferred_df$true)/all_inferred_df$true

diff_frac_shifted_top <- (all_inferred_df$shift.975-all_inferred_df$true)/all_inferred_df$true
diff_frac_shifted_bottom <- (all_inferred_df$shift.025-all_inferred_df$true)/all_inferred_df$true

diff_frac_RL_top <- (all_inferred_df$inferred.975-all_inferred_df$true)/all_inferred_df$true
diff_frac_RL_bottom <- (all_inferred_df$inferred.025-all_inferred_df$true)/all_inferred_df$true

diff_frac_df <- data.frame(c(obs_1 - length(p) + 1):obs_2, diff_frac_shifted_bottom, diff_frac_shifted, diff_frac_shifted_top, diff_frac_RL_bottom, diff_frac_RL, diff_frac_RL_top)
names(diff_frac_df) <- c('time', "diff_frac_shifted.025", "diff_frac_shifted.median", "diff_frac_shifted.975", "diff_frac_RL.025", "diff_frac_RL.median", "diff_frac_RL.975")

diff_frac_df %>%
  ggplot() +
  geom_line(aes(x = time, y = diff_frac_shifted, col = "Median Infections, Shifted", lty = "Median Infections, Shifted", legend = "Median Infections, Shifted"), size = .6)  +
  geom_ribbon(aes(x = time, ymin = diff_frac_shifted.025, ymax = diff_frac_shifted.975, fill = "95% CI, shifted", legend = "95% CI, shifted"), alpha = 0.4) +
  geom_line(aes(x = time, y = diff_frac_RL.median, col = "Median Infections, Inferred", lty = "Median Infections, Inferred", legend = "Median Infections, Inferred"), size = .6)  +
  geom_ribbon(aes(x = time, ymin = diff_frac_RL.025, ymax = diff_frac_RL.975, fill = "95% CI, inferred", legend = "95% CI, inferred"), alpha = 0.4) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, col = "Intervention Time", lty = "Intervention Time", legend = "Intervention Time"), size = .5) +
  scale_color_manual(values = c("black", "red", "blue", "black")) + 
  scale_linetype_manual(values = c("dotted", "solid", "solid", "dotdash")) +
  labs(linetype = "", colour = "", fill = "") +
  ylab("count")+
  xlab("time (days)") +
  coord_cartesian(xlim=c(0, obs_2), ylim=c(-1, 5)) + 
  ggtitle('Shifting vs. RL inference of case counts, fractional error') -> shift_RL_fractional_difference_with_CI

shift_RL_fractional_difference_with_CI

saveRDS(shift_RL_fractional_difference_with_CI, sprintf("%s/shift_RL_fractional_difference_with_CI.Rds", intervention_file_name))
ggsave(sprintf("%s/shift_RL_fractional_difference_with_CI.png", intervention_file_name))
...
# plot_grid(shift_RL_true_with_CI, shift_RL_fractional_difference_with_CI, labels = "AUTO", ncol = 1, align = 'v') -> all_plot
# 
# all_plot
# 
...
## Added 12/23/20: $R_t$ calculation
...
```{r}
all_u_df <- data.frame("time" = all_inferred_df$time, "true" = all_inferred_df$true, "count_val" = t(all_u[seq(8000, 10000, 50),]))
#dim(all_u_df)

all_u_df_counts <- dplyr::select(all_u_df, starts_with("count"))
#apply(all_u_df_counts, 2, function(x){print(x)})
all_u_df_cori <-  lapply(names(all_u_df_counts), function(x){get_cori(df.in = all_u_df, icol_name = x)})

cori_shifted <- get_cori(df.in = all_inferred_df, icol_name = "shift.median") #actually want to apply to a bunch of samples from the shifted time series distribution, similar to the all_u example which is sampling from the bayesian time series distribution
cori_true <- get_cori(df.in = all_inferred_df, icol_name = "true")
```
```{r}
all_u_df_cori_bound <- bind_rows(all_u_df_cori, .id="sample_num")
all_u_df_cori_bound$sample_num <- as.factor(as.integer(all_u_df_cori_bound$sample_num))
all_u_df_cori_bound$time <- as.numeric(all_u_df_cori_bound$time)
all_u_df_cori_bound %>%
  group_by(time) %>% 
  summarise(cori.val = mean(Cori.mean), cori.025.val = min(Cori.025), cori.975.val = max(Cori.975)) -> all_u_df_cori_summary
```

```{r}
all_u_df_cori_bound %>%
  filter(!is.na(Cori.mean) & !is.na(Cori.mean)) %>%
  ggplot() + 
  geom_hline(aes(yintercept = 1, linetype = "Rt=1", color = "Rt=1")) +
  geom_line(aes(x = time, y = Cori.mean, color = "Sample Rt", linetype = "Sample Rt")) +
  geom_ribbon(aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = "Sample Rt"), alpha=0.4, show.legend = FALSE) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, color = 'Intervention time', linetype = 'Intervention time')) + 
  ...
  coord_cartesian(xlim=c(0, obs_2), ylim=c(-.01, 5)) +
  xlab("time (days)") + 
  ylab("Rt") +
  ggtitle('Calculations of Rt') ->all_u_df_cori_plot

#all_u_df_cori_plot

ggsave(sprintf("%s/all_u_df_cori_plot.png", intervention_file_name))
```
```{r}
all_u_df_cori_summary %>%
  filter(!is.na(cori.val) & !is.na(cori.025.val) & !is.na(cori.975.val)) %>%
  ggplot() + 
  geom_hline(aes(yintercept = 1, linetype = "Rt=1", color = "Rt=1")) +
  geom_line(aes(x = time, y = cori.val, color = "Sample Rt mean", linetype = "Sample Rt mean")) +
  geom_ribbon(aes(x = time, ymin=cori.025.val, ymax = cori.975.val, fill = "Sample Rt mean"), alpha=0.4, show.legend = FALSE) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, color = 'Intervention time', linetype = 'Intervention time')) + 
  scale_linetype_manual(values=c("dotted", "dashed", "solid")) + 
  scale_color_manual(values = c("black", "black", "red")) +
  scale_fill_manual(values = c("red")) +
  ...
  xlab("time (days)") + 
  ylab("Rt") +
  ggtitle('Calculations of Rt') ->all_u_df_cori_summary_plot

#all_u_df_cori_summary_plot

ggsave(sprintf("%s/all_u_df_cori_summary_plot.png", intervention_file_name))
```
...
cori_shifted %>%
  ...
  ggplot() + 
  geom_hline(aes(yintercept = 1, linetype = "Rt=1", color = "Rt=1")) +
  geom_line(aes(x = time, y = Cori.mean, color = "Shifted cases Rt", linetype = "Sample Rt")) +
  geom_ribbon(aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = "Shifted cases Rt"), alpha=0.4, show.legend = FALSE) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, color = 'Intervention time', linetype = 'Intervention time')) + 
  scale_linetype_manual(values=c("dotted", "dashed", "solid")) + 
  ...
  xlab("time (days)") + 
  ylab("Rt") +
  ggtitle('Calculations of Rt') ->cori_shifted_plot

#cori_shifted_plot

ggsave(sprintf("%s/cori_shifted_plot.png", intervention_file_name))
```
```{r}
cori_true %>%
  filter(!is.na(Cori.mean) & !is.na(Cori.mean)) %>%
  ggplot() + 
  geom_hline(aes(yintercept = 1, linetype = "Rt=1", color = "Rt=1")) +
  geom_line(aes(x = time, y = Cori.mean, color = "True infections Rt", linetype = "True infections Rt")) +
  geom_ribbon(aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = "True infections Rt"), alpha=0.4, show.legend = FALSE) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, color = 'Intervention time', linetype = 'Intervention time')) + 
  scale_linetype_manual(values=c("dotted", "dashed", "solid")) + 
  ...
  xlab("time (days)") + 
  ylab("Rt") +
  ggtitle('Calculations of Rt') ->cori_true_plot

#cori_true_plot

ggsave(sprintf("%s/cori_true_plot.png", intervention_file_name))
```
```{r}
all_u_df_cori_bound  %>%
  filter(!is.na(Cori.mean) & !is.na(Cori.mean)) -> all_u_df_cori_bound_filtered
cori_shifted %>%
  filter(!is.na(Cori.mean) & !is.na(Cori.mean)) -> cori_shifted_filtered
cori_true %>%
  filter(!is.na(Cori.mean) & !is.na(Cori.mean)) -> cori_true_filtered

ggplot() +
  geom_line(all_u_df_cori_summary, mapping = aes(x = time, y = cori.val, color = "Sample Rt mean", linetype = "Sample Rt mean")) +
  geom_ribbon(all_u_df_cori_summary, mapping= aes(x = time, ymin=cori.025.val, ymax = cori.975.val, fill = "Sample Rt mean"), alpha=0.4, show.legend = FALSE) +
  geom_line(cori_shifted_filtered, mapping = aes(x = time, y = Cori.mean, color = "Shifted cases Rt", linetype = "Shifted cases Rt")) +
  geom_ribbon(cori_shifted_filtered, mapping = aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = "Shifted cases Rt"), alpha=0.4, show.legend = FALSE) +
  geom_line(cori_true_filtered, mapping = aes(x = time, y = Cori.mean, color = "True infections Rt", linetype = "True infections Rt")) +
  geom_ribbon(cori_true_filtered, mapping = aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = "True infections Rt"), alpha=0.4, show.legend = FALSE) +
  geom_hline(aes(yintercept = 1, linetype = "Rt=1", color = "Rt=1")) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, color = 'Intervention time', linetype = 'Intervention time')) + 
  scale_linetype_manual(values=c("dotted", "dashed", "solid", "solid", "solid")) + 
  scale_color_manual(values = c("black", "black", "red", "blue", "black")) +
  ...
  xlab("time (days)") + 
  ylab("Rt") +
  ggtitle('Calculations of Rt') ->all_cori_plot

all_cori_plot
```

```{r}
#all_cori_df <- bind_rows(all_u_df_cori, cori_shifted, cori_true, .id="sample_num")
#all_cori_df$sample_num <- as.factor(all_cori_df$sample_num)
#all_cori_df$time <- as.numeric(all_cori_df$time)
#all_cori_df
```

```{r}
plot_grid(shift_RL_true_with_CI, shift_RL_fractional_difference_with_CI, all_cori_plot, labels = "AUTO", ncol = 1, align = 'v') -> all_plot
...
## Added 12/28/20: Sequentially cutting off time series

The aim of this section is to study whether or not the calculations of $R_t$ depend on when the time series is cut off. 
That is to say, suppose we compute $R_t$ using infection data up to a cutoff time  $t_c^{(1)}$.
Then, after more time passes, we may have access to a longer time series up to a later cutoff, $t_c^{(2)} > t_c^{(1)}$. 
We wish to know if the series of $R_t$ we obtain using the expanded time series cut off at $t_c^{(2)}$ is consistent with the series of $R_t$ we obtain using the time series cut off at $t_c^{(1)}$ for times less than the initial cutoff, $t_c^{(1)}$. 
This is relevant for an epidemic unfolding in real time, where researchers obtain more and more case data as time passes.

What we learn from the below calculation is that cutting off the time series \emph{after} doing the inference backcalculation using the entire observed case data does not affect the calculation of $R_t$ at subsequent times; that is, the Cori method is robust to cutting off the time series.
```{r}
t_seq <- seq(length(delay_distr_vec)+1, parlist$intervention_time_2, 14) #new data every other week, starting from just after the first delay distribution interval has passed

all_inferred_df$time <- as.numeric(all_inferred_df$time)

all_true_df <- dplyr::select(all_inferred_df, c("true","time"))
all_true_df

cut_off_time <- function(df, t_fin){
  df %>%
    filter(time < t_fin) -> df_cutoff
  #names(df_cutoff)[names(df_cutoff) == "true"] <- sprintf("true.%s", t_fin)
  df_cutoff
}

t_fin_test <- 5
sprintf("true.%s", t_fin_test)

all_true_cutoff_list <- lapply(t_seq, function(x){cut_off_time(all_true_df, x)})
#all_true_cutoff_list  

cori_true_list <- lapply(all_true_cutoff_list, function(x){get_cori(df.in = x, icol_name = "true")})

cori_true_list_bound <- bind_rows(cori_true_list, .id="cutoff_num")
cori_true_list_bound$cutoff_num <- factor(cori_true_list_bound$cutoff_num)
cori_true_list_bound$cutoff_num <- factor(cori_true_list_bound$cutoff_num, levels=rev(levels(cori_true_list_bound$cutoff_num))) 
cori_true_list_bound

```

```{r}
cori_true_list_bound %>% 
  #filter(as.integer(cutoff_num) < 10) %>%
  group_by(cutoff_num) %>%
  ggplot() + 
  geom_line(mapping = aes(x = time, y = Cori.mean, color = cutoff_num, linetype = cutoff_num)) +
  #geom_ribbon(mapping= aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = cutoff_num), alpha=0.4, show.legend = FALSE) +
  geom_hline(aes(yintercept = 1, linetype = "Rt=1", color = "Rt=1")) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, color = 'Intervention time', linetype = 'Intervention time')) + 
  scale_linetype_manual(values=c("solid", "solid", "solid", "solid", "solid", "solid", "dotted", "dashed")) + 
  #scale_color_manual(values = c("black", "black", "red", "yellow", "blue", "black")) +
  #scale_fill_manual(values = c("red", "blue", "black")) +
  #scale_color_discrete(name = "Legend", labels = c("Difference = 0", "Time of intervention", "Infections, inferred minus true")) +
  labs(color = "Legend", linetype = "Legend", fill = "Legend") +  
  #coord_cartesian(xlim=c(0, obs_2), ylim=c(-.01, 5)) +
  xlab("time (days)") + 
  ylab("Rt") +
  ggtitle('Calculations of Rt')


```

However, it may be that cutting off the \emph{case counts} before doing the backcalculation will change the \emph{inferred curve} of infections (using RL, for example). For example, this may be the case if the delay distribution is misspecified.
In this case, $R_t$ would certainly depend on when we cut off the time series, as the curve of infections we feed into the Cori method would depend on where we cut off the time series.
We test this idea below.

```{r}
sim_df$new_observed
#all_u <- get_samples_RL_method(sim_df$new_observed, delay_distr_vec)

all_obs_df <- dplyr::select(sim_df, c("time","new_observed"))
str(all_obs_df)
all_obs_df$new_observed


t_seq <- seq(3*(length(delay_distr_vec)+1), parlist$intervention_time_2, 100) #four values of t_seq, starting with a sequence that's long enough to run in bayesian function
length(t_seq)
t_seq

all_obs_df$time <- as.numeric(all_obs_df$time)
```
...
t_seq <- as.list(c(100, 150, 200))

#all_obs_cutoff_list <- lapply(t_seq, function(x){cut_off_time(all_obs_df, x)})
#all_obs_cutoff_list  
```

```{r}
all_obs_cutoff_samples <- lapply(t_seq, function(x){
  get_samples_RL_method(sim_df$new_observed, delay_distr_vec, obs_2_offset_in=(x - parlist$obs_1_offset-1))})

length(delay_distr_vec)
str(all_obs_cutoff_samples)
```
```{r}
p <- RL_delay(delay_distr_vec) #delay distribution vector - notation in keeping with previous notation. this changes the delays to be indexed from 0.
length(p)
...
obs_2 <- as.vector(t_seq) -parlist$obs_1_offset-1

obs_1
obs_2
obs_2 + length(p) #these are the lengths of the two different time series we've produced

u_quants_cutoff <- lapply(all_obs_cutoff_samples, function(x){colQuantiles(x[5000:10000,], probs = c(0, 0.025, .5, .975, 1))})
str(u_quants_cutoff)

test_list <- u_quants_cutoff[[1]]
test_list
length(test_list[,1])
plot(test_list[,3])

bayes_deconv_df_cutoff <- lapply(u_quants_cutoff, function(u_quants){data.frame("time" = 1:length(u_quants[,1]), "inferred.025" = u_quants[,2], "inferred.median" = u_quants[, 3], "inferred.975" = u_quants[, 4], "true" = sim_df$new_infected[1:length(u_quants[,1])])})
str(bayes_deconv_df_cutoff)

bayes_deconv_df_cutoff_bound <- bind_rows(bayes_deconv_df_cutoff, .id="cutoff_num")
str(bayes_deconv_df_cutoff_bound)
```

```{r}
bayes_deconv_df_cutoff_bound %>%
  #filter(as.integer(cutoff_num) < 3) %>%
  group_by(cutoff_num) %>%
  ggplot() + 
  geom_line(aes(x = time, y = inferred.median, col = cutoff_num, lty = cutoff_num, legend = cutoff_num), size = .8)  +
  geom_line(aes(x = time, y = true, col = "True Infections", lty = "True Infections", legend = "True Infections"), size = .8) + 
  geom_ribbon(aes(x = time, ymin = inferred.025, ymax = inferred.975, fill = cutoff_num, legend = cutoff_num), alpha = 0.2) +
  geom_vline(aes(xintercept = parlist$intervention_time_1, col = "Intervention Time", lty = "Intervention Time", legend = "Intervention Time"), size = 1.1) +
#  scale_color_manual(values = c("black", "red", "blue")) + 
#  scale_linetype_manual(values = c("dotted", "solid", "longdash")) +
#  labs(linetype = "", colour = "", fill = "") +
  ylab("count")+
  xlab("time (days)") + 
  ggtitle('Bayesian Deconvolution, time cutoffs') -> RL_inferred_plot_with_CI_sequential
RL_inferred_plot_with_CI_sequential

saveRDS(RL_inferred_plot_with_CI, sprintf("%s/RL_inferred_plot_with_CI_sequential.Rds", intervention_file_name))
ggsave(sprintf("%s/RL_inferred_plot_with_CI_sequential.png", intervention_file_name))
```
We find that in fact even when the delay distribution is misspecified, the Bayesian deconvolution is insensitive to cutting off the time series at different points. 
Since we already found that the same is true for the Cori method, we don't have to retest the Cori method using the Bayesian time series.
*Note: we have not tested misspecifying the serial interval distribution in this case; thus we can't definitively say that misspecifying the serial interval in particular (not just the overall delay distribution, which includes delays from observation which don't affect the Cori method) does not introduce a sensitivity to cutting off the time series at different points.*
