...
intervention_length_name <- "_testing_clean_code_uncertainty_highly_misspecified"
...
### Misspecification in observation distributions. A value of 1 means no misspecification. ###
      delay_distr_mean_mult = 1.8, #multiplier for how much to misspecify delay distribution mean - specify a single value
      delay_distr_std_mult = 1.8, #multiplier for how much to misspecy delay distribution std - specify a single value
...
ggplot() +
  geom_line(all_u_df_cori_bound_filtered, mapping = aes(x = time, y = Cori.mean, color = "Sample Rt", linetype = "Sample Rt")) +
  geom_ribbon(all_u_df_cori_bound_filtered, mapping = aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = "Sample Rt"), alpha=0.4, show.legend = FALSE) +
  geom_line(cori_shifted_filtered, mapping = aes(x = time, y = Cori.mean, color = "Shifted cases Rt", linetype = "Sample Rt")) +
...
```{r}
all_cori_df %>%
  filter(!is.na(Cori.mean) & !is.na(Cori.mean) & Cori.mean < 5 & Cori.975 < 8) %>%
  ggplot() + 
  geom_hline(aes(yintercept = 1, linetype = "one", color = "one")) +
  geom_line(aes(x = time, y = Cori.mean, color = sample_num, linetype = sample_num)) +
  geom_ribbon(aes(x = time, ymin=Cori.025, ymax = Cori.975, fill = sample_num), alpha=0.3, show.legend = FALSE) +
