...
intervention_length_name <- "_testing_clean_code_std_highly_misspecified"
...
```{r}
get_samples_RL_method <- function(obs_curve, delay_vec, obs_1_offset = parlist$obs_1_offset, obs_2_offset = parlist$obs_2_offset){
  ...
  obs_1 <- length(p)+obs_1_offset +1
  obs_2 <- obs_1 + obs_2_offset
...
#dftest <- data.frame(all_obs_cutoff_list[1])
#dftest$new_observed[dftest$new_observed > 1]
#test_samples<-get_samples_RL_method(sim_df$new_observed[1:190], delay_distr_vec) #for some reason which I'm not sure of right now, the new_observed curve length must be at least 190 in order for this to run
#test_samples
all_obs_cutoff_samples <- lapply(all_obs_cutoff_list, function(x){
  y <- data.frame(x)
  get_samples_RL_method(y$new_observed, delay_distr_vec)})
...
bayes_deconv_df_cutoff <- lapply(u_quants_cutoff, function(u_quants){data.frame("time" = (obs_1-length(p)-1+2):(obs_2), "inferred.025" = u_quants[,2], "inferred.median" = u_quants[, 3], "inferred.975" = u_quants[, 4], "true" = sim_df$new_infected[(obs_1-length(p)-1+2):(obs_2)])})
...
NOTE: Below code block is not ready for case of sequential time series inference.
```{r}
cori_true_list <- lapply(all_true_cutoff_list, function(x){get_cori(df.in = x, icol_name = "true")})
