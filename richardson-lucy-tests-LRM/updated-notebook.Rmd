...
all_u_df_names <- names(all_u_df)
all_u_df_names_counts <- names()

get_cori_all_u <- function(df_column){
  this_name <- name(df_column)
  get_cori(all_u_df, icol_name = this_name)
}

get_cori_all_inferred <- function(df_column){
  this_name <- name(df_column)
  get_cori(all_inferred_df, name = this_name)
}

all_u_df_counts <- select(all_u_df, num_range("X", 1:dim(all_u_df)[1]))
all_u_df_cori <-  lapply(all_u_df_names, function(x){get_cori(df.in = all_u_df, icol_name = x)})
  select(everything())

cori_df <- get_cori(df.in = all_u_df, icol_name = "count_val.7")

cori_df

cori_inferred <- get_cori(df.in = all_inferred_df, icol_name = "inferred.median")
cori_inferred
